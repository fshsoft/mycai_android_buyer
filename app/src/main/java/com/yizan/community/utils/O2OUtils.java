...
public final class O2OUtils {
    ...
    public static boolean isLogin(Context context) {
        try {
            ...
            e.printStackTrace();
            O2OUtils.clearUserData(context);
            return false;
        }
    }
...
//                PreferenceUtils.setValue(context, Constants.TOKEN, info.token);
    ...
    private static int sIsOpenProperty = -1;
    public static boolean isOpenProperty() {
        if(sIsOpenProperty != -1){
            return sIsOpenProperty == 1;
        }
        InitInfo initInfo = PreferenceUtils.getObject(YizanApp.getInstance().getApplicationContext(), InitInfo.class);
        if(initInfo == null){
            return false;
        }
        sIsOpenProperty = initInfo.isOpenProperty? 1: 0;
        return initInfo.isOpenProperty;
    }
}
