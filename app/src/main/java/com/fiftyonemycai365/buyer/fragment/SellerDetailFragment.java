...
import com.fiftyonemycai365.buyer.dialog.NoticeDialog;
...
public class SellerDetailFragment extends BaseFragment implements View.OnClickListener{
    ...
    private void initViewData(SellerInfo sellerInfo) {
        ...
        iv.setImageUrl(ImgUrl.squareUrl(R.dimen.image_height, mSellerInfo.logo), RequestManager.getImageLoader());
        iv.setDefaultImageResId(R.drawable.ic_default_circle);

        // 640 * 420
        RelativeLayout relativeLayout = mViewFinder.find(R.id.rl_head_container);
        ViewGroup.LayoutParams layoutParams = relativeLayout.getLayoutParams();
        if(layoutParams != null) {
            layoutParams.height = ImgUrl.getScreenWidth() * 420 / 640;
            relativeLayout.setLayoutParams(layoutParams);
        }
        if(!TextUtils.isEmpty(mSellerInfo.image)){
            iv = mViewFinder.find(R.id.iv_bg);
            iv.setImageUrl(ImgUrl.formatUrl(ImgUrl.getScreenWidth(), layoutParams.height, mSellerInfo.image), RequestManager.getImageLoader());
        }
